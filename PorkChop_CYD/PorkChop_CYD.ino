...
void setup() {
  ...
  // Init subsystems
  SFX::init();
  // Immediate hardware test — bypasses queue, fires ledc directly
  // If you hear this: ledc+amp are working, queue system may have bug
  // If silent: hardware issue (wiring, amp, pin)
  if (SFX::isLedcInited()) {
    Serial.println("[SFX] hardware test: 1000Hz/1500Hz");
    ledcChangeFrequency(SPEAKER_PIN, 1000, 10);
    ledcWrite(SPEAKER_PIN, 512);
    delay(250);
    ledcWrite(SPEAKER_PIN, 0);
    delay(80);
    ledcChangeFrequency(SPEAKER_PIN, 1500, 10);
    ledcWrite(SPEAKER_PIN, 512);
    delay(250);
    ledcWrite(SPEAKER_PIN, 0);
    Serial.println("[SFX] hardware test done");
  }
