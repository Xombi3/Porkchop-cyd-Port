...
void draw(T& dst, int yOff) {
  ...
  // Bouncing party pig — each line is center-datum'd independently since the
  // tail line ("z(    )") is a character wider than the other two; left-
  // aligning them to one x made the pig look lopsided.
  float bounce = sinf(elapsed / 220.0f) * 8.0f;
  int py = yOff + MAIN_H/2 - 40 + (int)bounce;
  dst.setTextSize(3);
  dst.setTextDatum(TC_DATUM);
  dst.setTextColor(TFT_BLACK, bg);
  dst.drawString(" ^  ^ ",  DISPLAY_W/2, py);
  dst.drawString("(^ 00)",  DISPLAY_W/2, py + 22);
  dst.drawString("z(    )", DISPLAY_W/2, py + 44);
