// ============================================================
// M5PORKCHOP → CYD (ESP32-2432S028R)  v5
// Original: https://github.com/0ct0sec/M5PORKCHOP by 0ct0
// CYD Port: adapted from full source
//
...
// in-app top bar uses its own leet-speak labels defined separately).
inline const char* porkchopModeName(PorkchopMode m) {
  switch (m) {
    ...
    case PorkchopMode::FILE_TRANSFER:  return "FILE TRANSFER";
    case PorkchopMode::CRASH_VIEWER:   return "CRASH VIEWER";
    case PorkchopMode::DIAGNOSTICS:    return "DIAGNOSTICS";
    case PorkchopMode::SWINE_STATS:    return "SWINE STATS";
    case PorkchopMode::BOAR_BROS:      return "BOAR BROS";
    case PorkchopMode::WIGLE_MENU:     return "WIGLE MENU";
    case PorkchopMode::UNLOCKABLES:    return "UNLOCKABLES";
    case PorkchopMode::BOUNTY_STATUS:  return "BOUNTY";
    case PorkchopMode::BACON_MODE:     return "BACON MODE";
    case PorkchopMode::PORK_PATROL:    return "PORK PATROL";
    case PorkchopMode::SNOUT_MODE:     return "SNOUT";
    case PorkchopMode::SWINE_RADAR:    return "SWINE RADAR";
    case PorkchopMode::SD_FORMAT:      return "SD FORMAT";
    case PorkchopMode::CHARGING:       return "CHARGING";
    case PorkchopMode::WEBUI_MODE:     return "WEB REMOTE";
    case PorkchopMode::PIG_PARTY:      return "PARTY PIG";
    case PorkchopMode::MATRIX_MODE:    return "THE MATRIX";
...
// SD also uses VSPI but on different pins (18/19/23/5)
// We must reclaim the VSPI bus before each SD operation since touchSPI shares it
SPIClass sdSPI(VSPI);
// SD bus reclaim — restores VSPI pins and re-registers SD after touchSPI reconfigures VSPI
// Call before any SD operation that happens after touch init
#define SD_RECLAIM() do { sdSPI.begin(18, 19, 23, SD_CS_PIN); SD.begin(SD_CS_PIN, sdSPI, 4000000); } while(0)
...
#define WEBUI_AP_PASS   "oinkoink1"  // WPA2-PSK — change this to your own password
...
static bool    achHistoryView = false; // false=catalog order, true=most-recently-unlocked-first
...
bool sdFoldersOk = false; // true only if /captures, /wigle, /logs all confirmed present
...
// Smooth rainbow wheel (0-255 -> RGB565) — used by the PARTY PIG easter egg.
inline uint16_t wheelColor(uint8_t pos) {
  pos = 255 - pos;
  uint8_t r, g, b;
...
// ============================================================
// AVATAR — original art and animations
// ============================================================
namespace Avatar {
...
// Speech bubble draw — original bubble positioning logic
...
// ============================================================
// IDLE CRITTERS — an occasional ambient cameo (cat, butterfly, or bird)
// that wanders across the idle screen every so often, purely cosmetic.
// Stays home during rain/thunder to stay consistent with the Weather mood.
// ============================================================
namespace Critters {

enum Kind : uint8_t { NONE = 0, CAT, BUTTERFLY, BIRD };
static Kind     active     = NONE;
static uint32_t startMs    = 0;
static uint32_t nextRollMs = 0;
static const uint32_t CROSS_MS = 7000; // time to cross the screen

static void maybeSpawn() {
  uint32_t now = millis();
  if (active != NONE || now < nextRollMs) return;
  nextRollMs = now + random(15000, 35000); // re-roll odds every 15-35s
  if (Weather::isRaining() || Weather::isThunderFlashing()) return; // stays in when stormy
  if (random(0, 100) < 30) {               // 30% chance per roll
    active  = (Kind)random(CAT, BIRD + 1);
    startMs = now;
  }
}
...
void draw() {
  ...
  if (elapsed > CROSS_MS) { active = NONE; return; }

  float t     = (float)elapsed / CROSS_MS;         // 0..1 across the screen
  int   critX = (int)(t * (DISPLAY_W + 100)) - 50;
  ...
  if (active == CAT) {
    float bob = sinf(elapsed / 130.0f) * 2.0f;
    mainSprite.drawString("=^..^=", critX, 150 + (int)bob);
  } else if (active == BUTTERFLY) {
    int  y       = 40 + (int)(sinf(elapsed / 180.0f) * 10.0f);
    bool wingsUp = ((elapsed / 150) % 2) == 0;
    mainSprite.drawString(wingsUp ? "-()-" : ".().", critX, y);
  } else if (active == BIRD) {
    int  y       = 25 + (int)(sinf(elapsed / 260.0f) * 6.0f);
    bool wingsUp = ((elapsed / 220) % 2) == 0;
    mainSprite.drawString(wingsUp ? "^--^" : "_--_", critX, y);
  }
}
...
// ── SECRET ACHIEVEMENTS (bank 2) ────────────────────────────────────────────
// The primary PorkAchievement bitmask above is fully allocated (64/64 bits),
// so hidden/easter-egg achievements get their own small bank rather than
// disturbing that packed enum. Entries here are always rendered as "???"
// in the achievements list until unlocked, since the point is to stumble
// onto them rather than see them spelled out as a checklist item.
enum PorkAchievement2 : uint64_t {
  ACH2_NONE         = 0,
  ACH2_PARTY_ANIMAL = 1ULL<<0,   // found the hidden PARTY PIG easter egg
  ACH2_RED_PILL     = 1ULL<<1,   // found the hidden MATRIX easter egg
  ACH2_DEBUG_MASTER = 1ULL<<2,   // rapid-tapped DIAGNOSTICS
};
struct AchDef2 { PorkAchievement2 flag; const char* name; const char* howTo; };
extern const AchDef2 ACH_LIST2[];
extern const uint8_t ACH_COUNT2;

// Unlock-history log — one record per achievement, indexed the same way the
// Achievements screen already combines both banks (bank1 index i, bank2 at
// ACH_COUNT+j). No RTC/GPS-time source on this hardware, so timestamps are
// recorded as (session number, seconds into that session) rather than wall
// clock — enough to answer "when, roughly" without needing real time.
#define MAX_ACH_HISTORY 64
struct AchUnlockRecord { uint16_t session; uint32_t atSec; }; // session==0 => never unlocked
...
namespace XP {
  ...
  // Secret achievements (bank 2)
  bool        hasAchievement2(PorkAchievement2 a);
  uint64_t    getAchievements2();
  uint8_t     getAchievementCount2();
  void        unlockAchievement2(PorkAchievement2 a);
  // Unlock history — combinedIdx follows the same i<ACH_COUNT (bank1) /
  // ACH_COUNT+j (bank2) convention used by the Achievements screen.
  AchUnlockRecord getUnlockRecord(uint8_t combinedIdx);
}

// Row count for the achievements screen's current view — catalog view shows
// everything; history view shows only entries with a recorded unlock
// timestamp (achievements earned before this feature existed won't have
// one, so they simply don't appear there).
uint8_t achViewCount() {
  uint8_t total = ACH_COUNT + ACH_COUNT2;
  if (!achHistoryView) return total;
  uint8_t c=0;
  for (uint8_t i=0;i<total;i++) {
    bool got = (i<ACH_COUNT) ? ((XP::getAchievements()&(uint64_t)ACH_LIST[i].flag)!=0)
                              : XP::hasAchievement2(ACH_LIST2[i-ACH_COUNT].flag);
    if (got && XP::getUnlockRecord(i).session>0) c++;
  }
  return c;
}
namespace OinkMode {
...
// Pig-face icon — used as favicon, apple-touch-icon, and the PWA manifest
// icon. SVG scales cleanly to any size a launcher/home-screen needs.
static const char ICON_SVG[] PROGMEM = R"SVG(<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 192 192">
<rect width="192" height="192" rx="28" fill="#0a0a0a"/>
<ellipse cx="96" cy="104" rx="62" ry="54" fill="#ffb020"/>
<path d="M50 62 L70 50 L74 80 Z" fill="#ffb020"/>
<path d="M142 62 L122 50 L118 80 Z" fill="#ffb020"/>
<circle cx="74" cy="92" r="8" fill="#0a0a0a"/>
<circle cx="118" cy="92" r="8" fill="#0a0a0a"/>
<ellipse cx="96" cy="128" rx="26" ry="18" fill="#e08000"/>
<circle cx="87" cy="128" r="5" fill="#0a0a0a"/>
<circle cx="105" cy="128" r="5" fill="#0a0a0a"/>
</svg>)SVG";

// PWA manifest — lets "Add to Home Screen" launch the page standalone
// (no browser chrome) with a real icon, instead of just bookmarking a tab.
static const char MANIFEST_JSON[] PROGMEM = R"JSON({
"name":"PorkChop","short_name":"PorkChop","start_url":"/","display":"standalone",
"background_color":"#0a0a0a","theme_color":"#0a0a0a",
"icons":[{"src":"/icon.svg","sizes":"any","type":"image/svg+xml","purpose":"any"}]
})JSON";

static const char INDEX_HTML[] PROGMEM = R"RAW(<!DOCTYPE html><html><head><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1"><meta http-equiv="Content-Security-Policy" content="upgrade-insecure-requests: 0"><title>PORKCHOP</title>
...
body{background:#0a0a0a;color:#ffb020;font:12px/1.4 'Courier New',monospace;text-align:center;margin:0;padding:8px;text-shadow:0 0 4px rgba(255,176,32,.45);overflow-x:hidden}
.scan{pointer-events:none;position:fixed;top:0;left:0;width:100%;height:100%;background:repeating-linear-gradient(0deg,rgba(0,0,0,.15) 0px,rgba(0,0,0,.15) 1px,transparent 1px,transparent 3px);z-index:999}
#boot{position:fixed;inset:0;background:#0a0a0a;color:#ffb020;display:flex;align-items:center;justify-content:center;font-size:22px;z-index:1000;transition:opacity .6s;letter-spacing:2px}
#boot.hide{opacity:0;pointer-events:none}
h1{font-size:16px;letter-spacing:3px;margin:6px 0}
.card{border:1px solid rgba(255,176,32,.4);border-radius:8px;background:#151008;box-shadow:0 0 12px rgba(255,176,32,.13);margin:8px auto;padding:8px;max-width:340px}
.cardhead{font-size:9px;letter-spacing:2px;color:rgba(255,176,32,.65);margin-bottom:6px;text-transform:uppercase}
canvas{width:320px;height:160px;image-rendering:pixelated;border:2px solid #ffb020;border-radius:4px;display:block;margin:0 auto;cursor:pointer}
.pigmascot{font-size:14px;line-height:1.15;color:#ffb020;animation:bob 1.8s ease-in-out infinite;display:inline-block;margin:2px 0;white-space:pre}
@keyframes bob{0%,100%{transform:translateY(0)}50%{transform:translateY(-4px)}}
.barwrap{background:#000;border:1px solid rgba(255,176,32,.35);border-radius:6px;height:14px;overflow:hidden;margin:6px auto;max-width:280px}
.barfill{height:100%;width:0%;background:linear-gradient(90deg,#ff7a00,#ffd000);transition:width .6s}
.chips{display:flex;flex-wrap:wrap;gap:4px;justify-content:center;margin:6px 0}
.chip{background:#000;border:1px solid rgba(255,176,32,.3);border-radius:5px;padding:3px 6px;font-size:9px;color:#7dff7d}
button{background:#1c1608;color:#ffb020;border:1px solid rgba(255,176,32,.5);border-radius:6px;padding:6px 10px;margin:2px;font:11px monospace;cursor:pointer;transition:transform .1s;text-shadow:none}
button:active{transform:scale(.93);background:#2a2010}
.danger{color:#ff5c5c;border-color:rgba(255,92,92,.55)}
.good{color:#7dff7d;border-color:rgba(125,255,125,.5)}
#fb{display:none;margin:6px auto;max-width:320px;text-align:left;font-size:10px}
#flist div{padding:3px 0;border-bottom:1px solid rgba(255,176,32,.15)}
#confettiWrap{position:fixed;inset:0;pointer-events:none;overflow:hidden;z-index:998}
.confetti{position:absolute;top:-10px;width:6px;height:6px;border-radius:1px;animation:fall 1.6s linear forwards}
@keyframes fall{to{transform:translateY(110vh) rotate(360deg);opacity:0}}
#achBanner{position:fixed;top:10px;left:50%;transform:translateX(-50%) translateY(-70px);background:#151008;border:1px solid #ffd000;box-shadow:0 0 16px rgba(255,208,0,.65);padding:8px 16px;border-radius:8px;font-size:12px;color:#ffd000;transition:transform .4s;z-index:1000;white-space:nowrap}
#achBanner.show{transform:translateX(-50%) translateY(0)}
</style></head>
<body>
<div class=scan></div>
<div id=boot>OINK OINK</div>
<div id=confettiWrap></div>
<div id=achBanner>&#127942; ACHIEVEMENT UNLOCKED</div>

<h1>&#128055; PORKCHOP</h1>
<pre id=pig class=pigmascot>  ^   ^  
( o   o )
  (0 0)  </pre>

<div class=card>
<div class=cardhead>Live Screen</div>
<canvas id=c width=160 height=80></canvas>
<div id=s style="color:#666;font-size:8px;margin-top:4px">connecting</div>
</div>

<div class=card>
<div class=cardhead>Status</div>
<div class=barwrap><div class=barfill id=xpbar></div></div>
<div id=lvltxt style="font-size:10px;margin-bottom:4px">LVL --</div>
<div class=chips id=chips></div>
</div>

<div class=card>
<div class=cardhead>D-Pad</div>
<div><button onclick=g('tap_top')>&#9650; UP</button></div>
<div><button onclick=g('tap_l')>&#9664; L</button><button onclick=g('tap_c')>&#9679; MID</button><button onclick=g('tap_r')>&#9654; R</button></div>
<div><button onclick=g('tap_bot')>&#9660; DOWN</button><button onclick=g('hold')>&#9201; HOLD</button></div>
</div>

<div class=card>
<div class=cardhead>Modes</div>
<button onclick=g('oink') class=danger>&#128055; OINK</button>
<button onclick=g('dnh')>&#129323; DNH</button>
<button onclick=g('warhog')>&#128225; HOG</button>
<button onclick=g('bacon')>&#129382; BACON</button>
<button onclick=g('mode_idle')>&#128564; IDLE</button>
<button onclick=g('menu')>&#128203; MENU</button>
</div>

<div class=card>
<div class=cardhead>Tools &amp; Uploads</div>
<button onclick=g('stats')>&#128202; STATS</button>
<button onclick=g('captures')>&#128451; CAPS</button>
<button onclick=g('diag')>&#129658; DIAG</button>
<button onclick=g('settings')>&#9881; CFG</button>
<button onclick=g('wigle_up') class=good>&#9729; WIGLE</button>
<button onclick=g('wpasec_up') class=good>&#9729; WPA</button>
</div>

<div class=card>
<div class=cardhead>Files</div>
<button onclick=toggleFB()>&#128193; BROWSE SD</button>
<div id=fb>
<div id=fbpath style="color:#ffb020;margin:6px 0;word-break:break-all">/</div>
<div id=flist></div>
</div>
</div>
...
var curDir='/', lastAch=null, pigEl=document.getElementById('pig');
var FACES={hype:'( *   * )',good:'( ^   ^ )',ok:'( o   o )',meh:'( -   - )',sad:'( ;   ; )'};
var CONFETTI_COLORS=['#ff5c5c','#ffb020','#ffe066','#7dff7d','#5ad1ff','#c77dff'];

// Boot sequence — mirrors the device's own splash screens
setTimeout(function(){document.getElementById('boot').textContent='MY NAME IS';},700);
setTimeout(function(){document.getElementById('boot').innerHTML='&#128055; PORKCHOP';},1400);
setTimeout(function(){document.getElementById('boot').classList.add('hide');},2100);

function celebrate(){
  var b=document.getElementById('achBanner');
...
function toggleFB(){var e=document.getElementById('fb');if(e.style.display==='none'||!e.style.display){e.style.display='block';loadDir(curDir);}else{e.style.display='none';}}
function loadDir(dir){curDir=dir;fetch(H+'/files?dir='+encodeURIComponent(dir),{cache:'no-store'}).then(function(r){return r.text();}).then(function(t){renderDir(dir,t);}).catch(function(){document.getElementById('flist').textContent='error loading';});}
function renderDir(dir,t){
  ...
  if(dir!=='/'){
    ...
    up.textContent='.. (up)';
    up.style.cssText='cursor:pointer;color:#7dff7d;padding:2px 0';
    up.onclick=function(){var parts=dir.split('/').filter(Boolean);parts.pop();loadDir('/'+parts.join('/'));};
    list.appendChild(up);
  }
  t.split('\n').forEach(function(line){
    if(!line)return;
    var parts=line.split('|');
    if(parts[0]==='ERR'){var e=document.createElement('div');e.textContent=parts[1]||'error';list.appendChild(e);return;}
    var type=parts[0],name=parts[1],size=parts[2];
    var full=(dir==='/'?'':dir)+'/'+name;
    var row=document.createElement('div');
    row.style.padding='2px 0';
    if(type==='D'){
      row.textContent='&#128193; '+name;
      row.style.cssText='cursor:pointer;color:#7dff7d;padding:2px 0';
      row.onclick=function(){loadDir(full);};
    } else {
      var link=document.createElement('a');
      link.href=H+'/download?path='+encodeURIComponent(full);
      link.setAttribute('download',name);
      link.textContent=name+' ('+size+'B)';
      link.style.color='#ffb020';
      row.appendChild(link);
    }
    list.appendChild(row);
  });
}
function p(){fetch(H+'/screen',{cache:'no-store'}).then(function(r){return r.arrayBuffer();}).then(function(b){var h=b.byteLength/320,v=new DataView(b),img=x.createImageData(160,h),px=img.data;for(var i=0;i<160*h;i++){var pv=v.getUint16(i*2,false);px[i*4]=(pv>>8)&248;px[i*4+1]=(pv>>3)&252;px[i*4+2]=(pv<<3)&248;px[i*4+3]=255;}x.putImageData(img,0,0);s.textContent='live';setTimeout(p,700);}).catch(function(e){s.textContent='err:'+e.message;setTimeout(p,1500);});}
function u(){fetch(H+'/status',{cache:'no-store'}).then(function(r){return r.text();}).then(function(t){
  var o={};t.split('\n').forEach(function(l){var i=l.indexOf('=');if(i>0)o[l.slice(0,i)]=l.slice(i+1);});
  document.getElementById('lvltxt').textContent='LVL '+o.level+'  \u2014  '+o.progress+'% to next  ['+(o.mood||'ok').toUpperCase()+']';
  document.getElementById('xpbar').style.width=(o.progress||0)+'%';
  var face=FACES[o.mood]||FACES.ok;
  pigEl.textContent='  ^   ^  \n'+face+'\n  (0 0)  ';
  var chips=document.getElementById('chips');
  chips.innerHTML='';
  [['MODE',o.mode],['NETS',o.nets],['HS',o.hs],['PMKID',o.pmkid],['DEAUTH',o.deauth],['GPS',o.gps],['HEAP',o.heap],['UP',o.uptime]].forEach(function(kv){
    var d=document.createElement('div');d.className='chip';d.textContent=kv[0]+':'+kv[1];chips.appendChild(d);
  });
  var ach=parseInt(o.ach,10);
  if(!isNaN(ach)){ if(lastAch!==null && ach>lastAch) celebrate(); lastAch=ach; }
  setTimeout(u,700);
}).catch(function(){setTimeout(u,1500);});}
var actx = null;
function beep(freq1, freq2, dur, type){
  try {
...
function playClick(){ beep(900, 700, 0.05, 'square'); }
function playOink(){ beep(320, 140, 0.18, 'sawtooth'); }
function g(cmd){
  if (cmd==='oink') playOink(); else playClick();
  fetch(H+'/cmd?c='+cmd,{cache:'no-store'}).catch(function(){setTimeout(function(){fetch(H+'/cmd?c='+cmd,{cache:'no-store'}).catch(function(){});},250);});
}
c.addEventListener('click',function(e){var r=c.getBoundingClientRect();g('tap_xy_'+Math.round(e.clientX-r.left)+'_'+Math.round(e.clientY-r.top+20));});
setTimeout(p,1000);
setTimeout(u,500);
...
// ============================================================
// EASTER EGG: PARTY PIG — celebration animation + RGB LED lights
// Unlocked by the PartyPig gesture tracker above. Kept deliberately gentle
// (smooth color drift, LED changes capped well under 3Hz) rather than a
// hard strobe, since screen flashing can be a photosensitivity risk.
// ============================================================
namespace PartyPigFX {

#define PARTYPIG_CONFETTI_COUNT 16
struct Confetto { int16_t x, y; uint8_t speed; uint8_t hue; };
static Confetto bits[PARTYPIG_CONFETTI_COUNT];
static uint32_t startMs   = 0;
static bool     ledActive = false;

void start() {
  startMs = millis();
  for (int i = 0; i < PARTYPIG_CONFETTI_COUNT; i++) {
    bits[i].x     = random(0, DISPLAY_W);
    bits[i].y     = random(-MAIN_H, MAIN_H);
    bits[i].speed = random(2, 6);
    bits[i].hue   = random(0, 256);
  }
  ledActive = true;
}
...
// Cycles the RGB LED through red/green/blue roughly twice a second —
// festive, but well under flicker-risk thresholds.
void updateLED(uint32_t elapsed) {
  if (!ledActive) return;
  uint8_t phase = (elapsed / 500) % 3;
  digitalWrite(LED_R_PIN, phase == 0 ? LOW : HIGH);
  digitalWrite(LED_G_PIN, phase == 1 ? LOW : HIGH);
  digitalWrite(LED_B_PIN, phase == 2 ? LOW : HIGH);
}
...
void draw(T& dst, int yOff) {
  ...
  // Slow, smooth rotating rainbow backdrop — a drift, not a strobe.
  uint8_t bgHue = (elapsed / 20) % 256;
  uint16_t bg = wheelColor(bgHue);
  dst.fillRect(0, yOff, DISPLAY_W, MAIN_H, bg);

  // Falling confetti
  for (int i = 0; i < PARTYPIG_CONFETTI_COUNT; i++) {
    bits[i].y += bits[i].speed / 3 + 1;
    if (bits[i].y > MAIN_H) { bits[i].y = -8; bits[i].x = random(0, DISPLAY_W); }
    dst.fillRect(bits[i].x, yOff + bits[i].y, 4, 4, wheelColor(bits[i].hue));
  }

  // Cat cameo — strolls across the floor on a loop to join the party.
  // Position is derived purely from elapsed time (a sawtooth), so it needs
  // no persistent per-frame state; it wraps while off-screen, so the loop
  // is invisible.
  const uint32_t catPeriod = 6000;              // ms per crossing
  const int32_t  catSpan   = DISPLAY_W + 100;    // travel range, off-screen both ends
  int32_t catX = (int32_t)((elapsed % catPeriod) * (uint32_t)catSpan / catPeriod) - 70;
  float catBob = sinf(elapsed / 130.0f) * 3.0f;
  int catY = yOff + MAIN_H - 70 + (int)catBob;
  dst.setTextSize(2);
  dst.setTextDatum(TC_DATUM);
  dst.setTextColor(TFT_WHITE, bg);
  dst.drawString(" /\\_/\\ ", catX, catY);
  dst.drawString("( o.o )", catX, catY + 16);
  dst.drawString(" > ^ < ", catX, catY + 32);

  // Party pig — the app's regular pig sprite (same FRAMES_R/L ASCII frames
  // and tail-offset logic used everywhere else, e.g. Avatar::draw()), just
  // dancing: it sways side to side instead of the usual vertical bounce,
  // and turns to face whichever way it's currently swaying.
  const int   pigLineH  = 22;
  const int   bobRange  = 46;
  float       bobPhase  = elapsed / 300.0f;
  int         startX    = (DISPLAY_W/2 - 63) + (int)(sinf(bobPhase) * bobRange);
  int         startY    = yOff + MAIN_H/2 - pigLineH;
  bool        faceRight = cosf(bobPhase) >= 0.0f;   // face the direction of travel
  const char** frame    = faceRight ? Avatar::FRAMES_R[2] : Avatar::FRAMES_L[2]; // EXCITED
  dst.setTextSize(3);
  dst.setTextDatum(TL_DATUM);
  dst.setTextColor(colorFG(), bg);
  dst.drawString(frame[0], startX, startY);
  dst.drawString(frame[1], startX, startY + pigLineH);
  char bodyLine[16];
  strncpy(bodyLine, faceRight ? "z(    )" : "(    )z", sizeof(bodyLine));
  int bodyX = faceRight ? (startX - 18) : startX;
  dst.drawString(bodyLine, bodyX, startY + pigLineH * 2);

  // Title + hint
  dst.setTextDatum(TC_DATUM);
  dst.setTextSize(2);
  dst.setTextColor(TFT_WHITE, bg);
  dst.drawString("PARTY PIG!!", DISPLAY_W/2, yOff + 16);
  dst.setTextSize(1);
  dst.drawString("TAP OR HOLD TO CALM DOWN", DISPLAY_W/2, yOff + MAIN_H - 14);
}

} // namespace PartyPigFX

// ============================================================
// EASTER EGG: THE MATRIX — digital rain + flashing wake-up message.
// Unlocked by the RedPill gesture tracker above. A deliberately different
// kind of payoff than PartyPigFX: quiet and moody instead of loud and
// colorful, no LED involvement, message fades rather than strobes.
// ============================================================
namespace MatrixFX {

#define MATRIX_COLS 16
struct MatrixDrop { int16_t y; uint8_t speed; uint8_t len; };
static MatrixDrop drops[MATRIX_COLS];
static uint32_t startMs = 0;
...
void draw(T& dst, int yOff) {
  uint32_t elapsed = millis() - startMs;
  dst.fillRect(0, yOff, DISPLAY_W, MAIN_H, TFT_BLACK);

  dst.setTextSize(1);
  dst.setTextDatum(TL_DATUM);
  int colW = DISPLAY_W / MATRIX_COLS;
  for (int c=0; c<MATRIX_COLS; c++) {
    drops[c].y += drops[c].speed;
    if (drops[c].y - drops[c].len*10 > MAIN_H) {
      drops[c].y     = random(-60, 0);
      drops[c].speed = random(2, 6);
      drops[c].len   = random(4, 9);
    }
    for (int k=0; k<drops[c].len; k++) {
      int py = drops[c].y - k*10;
      if (py < 0 || py > MAIN_H) continue;
      char ch = (char)('0' + random(0,10));
      if (random(0,3)==0) ch = (char)('A' + random(0,26));
      int g = 200 - k*22; if (g < 20) g = 20;
      uint16_t col = (k==0) ? TFT_WHITE : tft.color565(0, g, 0);
      dst.setTextColor(col, TFT_BLACK);
      char s[2] = {ch,0};
      dst.drawString(s, c*colW + colW/4, yOff + py);
    }
  }

  // Message fades in/out slowly rather than hard-flashing (photosensitivity)
  uint32_t cyc = elapsed % 6000;
  if (cyc < 3000) {
    dst.setTextDatum(TC_DATUM);
    dst.setTextColor(TFT_WHITE, TFT_BLACK);
    dst.setTextSize(2);
    dst.drawString("WAKE UP, PIG...", DISPLAY_W/2, yOff + MAIN_H/2 - 10);
    dst.setTextSize(1);
    dst.drawString("TAP OR HOLD TO UNPLUG", DISPLAY_W/2, yOff + MAIN_H/2 + 14);
  }
}
...
void drawTopBar() {
  ...
  switch (currentMode) {
    ...
    case PorkchopMode::PIG_PARTY:        modeBuf = "!!! PARTY !!!"; break;
    case PorkchopMode::MATRIX_MODE:      modeBuf = "W4K3 UP"; break;
  ...
  // Right: L{n} N:{nets} CH{ch} {up} [AP if WebUI running in the background]
  uint32_t upSec = (millis() - bootTime) / 1000;
  char right[56];
  snprintf(right, sizeof(right), "L%d N:%03d CH%02d %u:%02u%s",
...
void update() {
  ...
  // Mode-transition wipe — detected here rather than in enterMode(), since
  // plenty of screens (MENU selections, WEBUI_MODE, easter eggs) set
  // currentMode directly and never pass through enterMode() at all. This
  // is the one point every mode change is guaranteed to cross every frame.
  static PorkchopMode _lastDrawnMode = PorkchopMode::IDLE;
  static uint32_t     _transStartMs  = 0;
  static bool         _transActive   = false;
  if (currentMode != _lastDrawnMode) {
    _lastDrawnMode = currentMode;
  ...
  switch (currentMode) {
    ...
    case PorkchopMode::IDLE: {
      if (useSprite) {
        ...
        Critters::draw();        // occasional cat/butterfly/bird cameo
    ...
    case PorkchopMode::ACHIEVEMENTS: {
      ...
      const uint8_t achTotal = ACH_COUNT + ACH_COUNT2;

      // Combined-index helpers spanning both achievement banks. Bank 2 is
      // secret achievements — name/howto stay masked as "???" until earned,
      // so browsing the list doesn't spoil what they're for.
      auto achGot = [&](uint8_t i)->bool {
        if (i < ACH_COUNT) return (XP::getAchievements() & (uint64_t)ACH_LIST[i].flag) != 0;
        return XP::hasAchievement2(ACH_LIST2[i - ACH_COUNT].flag);
      };
      auto achName = [&](uint8_t i)->const char* {
        if (i < ACH_COUNT) return ACH_LIST[i].name;
        return achGot(i) ? ACH_LIST2[i - ACH_COUNT].name : "???";
      };
      auto achHowTo = [&](uint8_t i)->const char* {
        if (i < ACH_COUNT) return (i < ACH_HOWTO_COUNT) ? ACH_HOWTO[i] : "???";
        return achGot(i) ? ACH_LIST2[i - ACH_COUNT].howTo : "???";
      };

      // Row->achievement mapping for the current view. Catalog view is the
      // identity mapping; history view reorders to most-recently-unlocked
      // first (by session, then seconds-into-session), including only
      // entries that actually have a recorded timestamp.
      static uint8_t viewOrder[MAX_ACH_HISTORY];
      uint8_t viewCount;
      if (achHistoryView) {
        viewCount = 0;
        for (uint8_t i=0; i<achTotal && i<MAX_ACH_HISTORY; i++) {
          if (achGot(i) && XP::getUnlockRecord(i).session > 0) viewOrder[viewCount++] = i;
        }
        for (uint8_t a=0; a<viewCount; a++) {
          uint8_t best=a;
          for (uint8_t b=a+1; b<viewCount; b++) {
            AchUnlockRecord rb=XP::getUnlockRecord(viewOrder[b]), rbest=XP::getUnlockRecord(viewOrder[best]);
            if (rb.session>rbest.session || (rb.session==rbest.session && rb.atSec>rbest.atSec)) best=b;
          }
          if (best!=a) { uint8_t t=viewOrder[a]; viewOrder[a]=viewOrder[best]; viewOrder[best]=t; }
        }
      } else {
        viewCount = achTotal;
        for (uint8_t i=0;i<achTotal && i<MAX_ACH_HISTORY;i++) viewOrder[i]=i;
      }
      auto rowToCombined = [&](uint8_t row)->uint8_t { return (row<viewCount) ? viewOrder[row] : 0; };

      // Detail popup (toast style — FG bg, BG text)
      auto drawAchDetail = [&](auto& dst, int yOff) {
        uint8_t i = rowToCombined(achDetailIdx);
        bool got = achGot(i);
        const char* howTo = achHowTo(i);
        ...
        dst.setTextColor(colorBG(),colorFG());
        dst.setTextDatum(TC_DATUM);
        dst.drawString(got ? achName(i) : "UNKNOWN", DISPLAY_W/2, bY+8);
        dst.drawString(got ? "UNLOCKED" : "LOCKED",  DISPLAY_W/2, bY+22);
        // Word-wrap howTo (2 lines — the 3rd line's space is used for the
        // unlock-history line below when this achievement's been earned)
        ...
        if (got) {
          AchUnlockRecord rec = XP::getUnlockRecord(i);
          if (rec.session > 0) {
            char ub[40];
            uint32_t h = rec.atSec/3600, m = (rec.atSec/60)%60;
            if (h > 0) snprintf(ub,sizeof(ub),"SESSION #%u @ %luH%02lum IN",rec.session,(unsigned long)h,(unsigned long)m);
            else       snprintf(ub,sizeof(ub),"SESSION #%u @ %lum IN",rec.session,(unsigned long)m);
            dst.drawString(ub, cx, textY+2*lh);
          }
        }
        dst.setTextDatum(TL_DATUM);
      };
      ...
      auto drawAch = [&](auto& dst, int yOff) {
        ...
        // Header
        dst.setTextDatum(TC_DATUM);
        char hdr[48]; snprintf(hdr,sizeof(hdr),"PR00F %d/%d  L%d%s",
          XP::getAchievementCount() + XP::getAchievementCount2(), achTotal, XP::getLevel(),
          achHistoryView ? "  [HIST]" : "");
        ...
        for (uint8_t r=0; r<ROWS; r++) {
          uint8_t row = achScroll + r;
          if (row >= viewCount) break;
          uint8_t i = rowToCombined(row);
          bool got = achGot(i);
          int ry = listY + r*lh;
          bool sel = (row == achDetailIdx);
          if (sel) {
          ...
          dst.setTextDatum(TL_DATUM);
          char row_[36]; snprintf(row_,sizeof(row_),"%s %s", got?"[X]":"[ ]", achName(i));
          dst.drawString(row_, 4, ry+3);
        }
        dst.setTextColor(colorFG(), colorBG());
        if (achScroll > 0)               { dst.setTextDatum(TR_DATUM); dst.drawString("^", DISPLAY_W-2, listY+2); }
        if (achScroll+ROWS < viewCount)  { dst.setTextDatum(TR_DATUM); dst.drawString("v", DISPLAY_W-2, listY+(ROWS-1)*lh+2); }
        dst.setTextDatum(TC_DATUM);
        dst.drawString("TAP HEADER: history  HOLD:back", DISPLAY_W/2, yOff+MAIN_H-10);
      };
    ...
    case PorkchopMode::DIAGNOSTICS: {
      auto drawDiag = [&](auto& dst, int yOff) {
        ...
        if (sdAvailable) {
          snprintf(b,sizeof(b),"SD:%lluMB free%s", (SD.totalBytes()-SD.usedBytes())/(1024*1024),
                   sdFoldersOk ? "" : " (folders!)");
        } else {
          strcpy(b,"SD: no card");
        }
    ...
    case PorkchopMode::WEBUI_MODE: {
      auto drawWebUI = [&](auto& dst, int yOff) {
        ...
        if (WebUI::isActive()) {
          ...
          dst.drawString("PASS:  " WEBUI_AP_PASS, 6, y); y+=dy+4;
          ...
          dst.drawLine(0, y, DISPLAY_W, y, colorFG()); y+=6;
          dst.drawString("TAP: stop   TAP CORNER: back", DISPLAY_W/2, y); y+=dy;
          dst.drawString("HOLD: stop + back", DISPLAY_W/2, y);
        } else {
          ...
          dst.drawLine(0, y, DISPLAY_W, y, colorFG()); y+=6;
          dst.drawString("TAP: start   TAP CORNER: back", DISPLAY_W/2, y);
        }
      };
    ...
    // ── PIG_PARTY (hidden easter egg) ────────────────────────────────────────
    case PorkchopMode::PIG_PARTY: {
      if (useSprite) PartyPigFX::draw(mainSprite, 0);
      else           PartyPigFX::draw(tft,        TOP_BAR_H);
      break;
    }

    // ── MATRIX_MODE (hidden easter egg) ─────────────────────────────────────
    case PorkchopMode::MATRIX_MODE: {
      if (useSprite) MatrixFX::draw(mainSprite, 0);
      else           MatrixFX::draw(tft,        TOP_BAR_H);
      break;
    }
  ...
  // ── Mode-transition wipe (covers the not-yet-revealed portion) ─────────────
  if (_transActive) {
    uint32_t elapsed = millis() - _transStartMs;
    const uint32_t TRANS_MS = 180;
    if (elapsed >= TRANS_MS) {
      _transActive = false;
    } else {
      int wipeX = (int)(((float)elapsed / TRANS_MS) * DISPLAY_W);
      int w = DISPLAY_W - wipeX;
      if (w > 0) {
        if (useSprite) mainSprite.fillRect(wipeX, 0, w, sprH, colorBG());
        else           tft.fillRect(wipeX, TOP_BAR_H, w, MAIN_H, colorBG());
      }
    }
  }
...
void showBootSplash() {
  ...
  // Screen 4: SD card warning — easy to miss if it's buried in Diagnostics,
  // and captures/logs silently won't be saved anywhere without one.
  if (!sdAvailable) {
    tft.fillScreen(0xF800); // red — deliberately loud, this matters
    tft.setTextColor(TFT_WHITE, 0xF800);
    tft.setTextSize(2);
    tft.drawString("NO SD CARD FOUND", DISPLAY_W/2, DISPLAY_H/2 - 20);
    tft.setTextSize(1);
    tft.drawString("CAPTURES & LOGS WON'T BE SAVED", DISPLAY_W/2, DISPLAY_H/2 + 8);
    tft.drawString("CHECK CARD IS SEATED + FAT32", DISPLAY_W/2, DISPLAY_H/2 + 22);
    delay(2500);
    tft.setTextColor(colorFG(), colorBG());
  } else if (!sdFoldersOk) {
    // Card mounted fine, but /captures, /wigle, or /logs couldn't actually
    // be created on it — same silent-failure risk as no card at all, just
    // with a different underlying cause (write-protect switch, corrupt
    // filesystem, full card).
    tft.fillScreen(0xF800);
    tft.setTextColor(TFT_WHITE, 0xF800);
    tft.setTextSize(2);
    tft.drawString("SD FOLDER SETUP FAILED", DISPLAY_W/2, DISPLAY_H/2 - 20);
    tft.setTextSize(1);
    tft.drawString("CAPTURES & LOGS MAY NOT SAVE", DISPLAY_W/2, DISPLAY_H/2 + 8);
    tft.drawString("CHECK WRITE-PROTECT + FREE SPACE", DISPLAY_W/2, DISPLAY_H/2 + 22);
    delay(2500);
    tft.setTextColor(colorFG(), colorBG());
  }
...
// ============================================================
// EASTER EGG: PARTY PIG gesture tracker
// In the MENU, tap the D-pad-style zones UP UP DOWN DOWN LEFT RIGHT LEFT
// RIGHT, then HOLD, to unlock a hidden celebration animation (see
// PartyPigFX below). Any stray input, or too long a pause between taps,
// resets progress — a classic Konami code, just built from touch zones.
// ============================================================
namespace PartyPig {

enum Dir : uint8_t { NONE = 0, UP, DOWN, LEFT, RIGHT };
static const Dir CODE[8] = { UP, UP, DOWN, DOWN, LEFT, RIGHT, LEFT, RIGHT };
static uint8_t  progress    = 0;
static uint32_t lastInputMs = 0;
static bool     armed       = false;  // sequence completed, waiting for the hold
#define PARTYPIG_INPUT_TIMEOUT_MS 3000

void feed(Dir d) {
  uint32_t now = millis();
  if (progress > 0 && now - lastInputMs > PARTYPIG_INPUT_TIMEOUT_MS) progress = 0;
  lastInputMs = now;
  if (d == CODE[progress]) {
    progress++;
    if (progress >= 8) { armed = true; progress = 0; }
  } else {
    progress = (d == CODE[0]) ? 1 : 0;
  }
}

// Returns true once, right when the hold following a completed code lands.
bool consumeArmed() {
...
// (see MatrixFX below) than PartyPig's UP UP DOWN DOWN LEFT RIGHT LEFT
// RIGHT. Both trackers watch every tap independently, so finding one
// doesn't interfere with finding the other.
// ============================================================
namespace RedPill {

using PartyPig::Dir;
using PartyPig::NONE; using PartyPig::UP; using PartyPig::DOWN;
using PartyPig::LEFT; using PartyPig::RIGHT;
static const Dir CODE[8] = { RIGHT, LEFT, RIGHT, LEFT, UP, UP, DOWN, DOWN };
static uint8_t  progress    = 0;
static uint32_t lastInputMs = 0;
static bool     armed       = false;
#define REDPILL_INPUT_TIMEOUT_MS 3000

void feed(Dir d) {
  uint32_t now = millis();
  if (progress > 0 && now - lastInputMs > REDPILL_INPUT_TIMEOUT_MS) progress = 0;
  lastInputMs = now;
  if (d == CODE[progress]) {
    progress++;
    if (progress >= 8) { armed = true; progress = 0; }
...
// Source: src/core/network_recon.cpp
// ============================================================
namespace NetworkRecon {
...
const AchDef ACH_LIST[] = {
  ...
  {ACH_MAX_LEVEL,      "MAX L3VEL",         "Reach level 150"},
...
const AchDef2 ACH_LIST2[] = {
  {ACH2_PARTY_ANIMAL, "P4RTY_AN1MAL", "Find the hidden party in the menu"},
  {ACH2_RED_PILL,     "R3D_P1LL",     "Wake up, pig..."},
  {ACH2_DEBUG_MASTER, "D3BUG_M4ST3R", "Tap diagnostics like you mean it"},
};
const uint8_t ACH_COUNT2 = sizeof(ACH_LIST2)/sizeof(ACH_LIST2[0]);
...
static const uint32_t XP_THRESH[150] = {
  ...
  871000,913000,956000,1000000,1045000,1091000,1138000,1186000,1235000,1285000,
  1336000,1388000,1441000,1495000,1550000,1606000,1663000,1721000,1780000,1840000,
  1901000,1963000,2026000,2090000,2155000,2221000,2288000,2356000,2425000,2495000,
  2566000,2638000,2711000,2785000,2860000,2936000,3013000,3091000,3170000,3250000,
  3331000,3413000,3496000,3580000,3665000,3751000,3838000,3926000,4015000,4105000,
  4196000,4288000,4381000,4475000,4570000,4666000,4763000,4861000,4960000,5060000,
  5161000,5263000,5366000,5470000,5575000,5681000,5788000,5896000,6005000,6115000,
  6226000,6338000,6451000,6565000,6680000,6796000,6913000,7031000,7150000,7270000,
  7391000,7513000,7636000,7760000,7885000,8011000,8138000,8266000,8395000,8525000,
  8656000,8788000,8921000,9055000,9190000,9326000,9463000,9601000,9740000,9880000
};

static const char* TITLES[150] = {
  ...
  "Z3R0D4Y Z0MB0AR","GH0ST P0RT PIG","STUXP1G","M3T4SPL0IT M4W","BURP SU1T3 B0AR",
  "SH3LLC0D3 SW1N3","T0R B4C0N","0N10N H0G","VPN V1P3R P1G","PR0XY P0RKCH0P",
  "D4RKW3B DR0V3","CR1MS0N C0D3R","BL4CKH4T B0AR","R3DT34M R00T3R","PWN B4C0N K1NG",
  "1NC0GN1T0 H4M","GH0STWR1T3R H0G","ST34LTH SN0UT","N1NJ4 N3TW0RK","CY83RSW1N3",
  "M4TR1X M4NG3R","K3RN3L P4N1C P1G","BUFF3R 0V3RFL0W","R00TSH3LL R4NCH3R","0MN1SC13NT 0INK",
  "P4YL04D PR0PH3T","F1RMW4R3 F4RM3R","SUP3RUS3R SW1N3","D0M41N 4DM1N H0G","GR1D M4ST3R",
  "SC4D4 SC0UT","S4TC0M SW1N3","L0RAWAN L0RD","M3SH M4ST3R","N4T10N ST4T3 P1G",
  "APT B4C0N","Z3R0 TRUST B0AR","41R G4P J0CK3Y","S1D3 CH4NN3L SN0UT","T3MP3ST H4M",
  "V4N ECK V1K1NG","RF SP00K","SDR SW1N3","GNU R4D10 GH0ST","QU4NTUM QU1NC3",
  "S1NGUL4R1TY SW1N3","0M3G4 0INK","L3G3ND4RY L4RD","M3G4H3RTZ M0N4RCH","B4C0N S1NGUL4R1TY",
  "V0ID W4LK3R H0G","EX4FLOOD P1G","DE3P PACK3T D3MON","C0GNIT0 C0CH0N","SW1N3 SING3NC3",
  "H0L0GR4M H0G","N3UTR1N0 N0S3","D4RKN3T D0N","BL4CK1C3 B0AR","TR0J4N TR0TT3R",
  "PH4NT0M PR0T0C0L","0BFUSC4T3D 0INK","P0LYM0RPH1C P1G","H4SH CR4CK3R H0G","CR4CK3D K3RN3L K1NG",
  "SUB N3T S0V3R31GN","W0RMH0L3 W4RH0G","1NC3PT10N INK","SYNT4X SW1N3","GH0ST 1N TH3 SH3LL P1G",
  "R3CURS1V3 R00T3R","F1REW4LL F1END","H0N3YP0T H0G","D33P F4K3 D0INK","4RCH1T3CT 0F CH40S",
  "0MNIV0R0US 0P3R4T0R","SUPR3M3 SW1N3H4CK3R","GR4ND M4ST3R H4M","1MM0RT4L 1NTRUS10N","P0RKCH0P PR1M3",
  "TH3 F1N4L F1R3W4LL","LEG3ND 0F TH3 L4N","APEX 4TT4CK SUR F4C3","M4ST3R 0F TH3 M3SH","APEX P0RKCH0P"
};
...
static uint64_t _achBits2 = 0;   // secret achievements (bank 2)
static AchUnlockRecord _achLog[MAX_ACH_HISTORY]; // zero-initialized: all "never unlocked"
...
void init() {
  ...
  _achBits2   = ((uint64_t)_xpPrefs.getUInt("ach2_hi",0)<<32) | _xpPrefs.getUInt("ach2_lo",0);
  if (_xpPrefs.getBytesLength("achlog") == sizeof(_achLog)) {
    _xpPrefs.getBytes("achlog", _achLog, sizeof(_achLog));
  } else {
    memset(_achLog, 0, sizeof(_achLog)); // first run, or size changed — start clean
  }
...
void save() {
  ...
  _xpPrefs.putUInt("ach2_lo",(uint32_t)(_achBits2 & 0xFFFFFFFF));
  _xpPrefs.putUInt("ach2_hi",(uint32_t)(_achBits2 >> 32));
  _xpPrefs.putBytes("achlog", _achLog, sizeof(_achLog));
...
void unlockAchievement(PorkAchievement a) {
  ...
  for (uint8_t i=0;i<ACH_COUNT;i++) {
    if (ACH_LIST[i].flag == a) {
      ...
      if (i < MAX_ACH_HISTORY) _achLog[i] = { (uint16_t)_sessions, (uint32_t)(millis()/1000) };
      break;
    }
  }
  // Check FULL_CLEAR
...
bool     hasAchievement2(PorkAchievement2 a)  { return (_achBits2 & (uint64_t)a) != 0; }
uint64_t getAchievements2()                   { return _achBits2; }
uint8_t  getAchievementCount2() {
  uint64_t b=_achBits2; uint8_t c=0;
  while(b){c+=b&1;b>>=1;} return c;
}

void unlockAchievement2(PorkAchievement2 a) {
  if (_achBits2 & (uint64_t)a) return; // already have it
  _achBits2 |= (uint64_t)a;
  SFX::play(SFX::ACHIEVEMENT);
  for (uint8_t i=0;i<ACH_COUNT2;i++) {
    if (ACH_LIST2[i].flag == a) {
      char buf[40]; snprintf(buf,sizeof(buf),"SECRET: %s",ACH_LIST2[i].name);
      Display::showToast(buf, 3500);
      Avatar::cuteJump();
      gain(NETWORK_FOUND); // small bonus XP for achievement
      uint8_t combined = ACH_COUNT + i;
      if (combined < MAX_ACH_HISTORY) _achLog[combined] = { (uint16_t)_sessions, (uint32_t)(millis()/1000) };
      break;
    }
  }
}
...
AchUnlockRecord getUnlockRecord(uint8_t combinedIdx) {
  if (combinedIdx >= MAX_ACH_HISTORY) return {0,0};
  return _achLog[combinedIdx];
}
...
void checkAchievements() {
  ...
  // Level achievement
  if(_level>=150)        unlockAchievement(ACH_MAX_LEVEL);
}

void addBLECount(uint32_t n) { _bleTotal+=n; checkAchievements(); }
const char* getTitle() { return _level<=150 ? TITLES[_level-1] : "APEX P0RKCH0P"; }
uint8_t getProgress() {
  if (_level>=150) return 100;
...
// Source: src/core/challenges.cpp
// ============================================================
...
// Source: src/core/heap_health.cpp + heap_policy.h
// ============================================================
  ...
  while(_hsR!=_hsW){
    ...
    if(fi<4&&hs.frames[fi].len==0){
      ...
      if(hs.hasValidPair()&&!hs.saved){
        ...
        strncpy(Display::lootSSID, hs.ssid, 32); Display::lootSSID[32] = '\0';
        hs.saved=true;
        SpiffsSave::saveHandshakeToSD(hs); // was previously RAM-only — lost on mode stop()
      }
    }
  }
}
...
void sessionStart() {
  ...
  if (!_logFile) {
    // Still failed even after ensuring the folder exists — fall back to
    // root rather than losing the session log entirely.
    snprintf(fname, sizeof(fname), "/tale_%06lu.txt", up);
    _logFile = SD.open(fname, FILE_WRITE);
  }
  if (!_logFile) { Serial.println("[TALES] log open failed — check SD card"); return; }
  _open = true;
...
void enterMode(PorkchopMode m) {
  ...
  switch(currentMode){
    ...
    case PorkchopMode::PIG_PARTY:       PartyPigFX::stop(); break;
  ...
  switch(m){
    ...
    default:
      // Ensure NetworkRecon is running in idle/menu states — but not while
      // WebUI's AP is up, since resuming it (isRunning() reports false
      // while paused, which is what pause() sets) would reintroduce the
      // exact channel-hop-kicks-the-AP-client bug WebUI::start() pauses it
      // to avoid in the first place.
      if(!WebUI::isActive() && !NetworkRecon::isRunning()) NetworkRecon::start();
      NetworkRecon::setPacketCallback(nullptr);
      break;
  }
}
...
void handleInput() {
  ...
  bool isWebInjected = false;
  if (touchEvent == 0) {
    if (webTapPending) {
      webTapPending = false;
      touchEvent = 1; tx = webTapX; ty = webTapY; isWebInjected = true;
    } else if (webHoldPending) {
      webHoldPending = false;
      touchEvent = 2; tx = DISPLAY_W/2; ty = TOP_BAR_H + MAIN_H/2; isWebInjected = true;
    }
  }
  ...
  switch (currentMode) {
    ...
    case PorkchopMode::MENU: {
      ...
      if (isTap) {
        ...
        if (PartyPig::consumeArmed()) {
          currentMode = PorkchopMode::PIG_PARTY;
          PartyPigFX::start();
          SFX::play(SFX::ULTRA_STREAK);
          XP::unlockAchievement2(ACH2_PARTY_ANIMAL);
        } else if (RedPill::consumeArmed()) {
          currentMode = PorkchopMode::MATRIX_MODE;
          MatrixFX::start();
          SFX::play(SFX::ULTRA_STREAK);
          XP::unlockAchievement2(ACH2_RED_PILL);
        } else {
          currentMode = PorkchopMode::IDLE;
          menuScroll = 0; menuSel = 0;
        }
    ...
    case PorkchopMode::ACHIEVEMENTS: {
      if (isTap) {
        if (achDetailShow) {
          achDetailShow = false;
        } else if (ty < TOP_BAR_H + 16) {
          // Header tap: toggle catalog order vs. most-recently-unlocked-first
          achHistoryView = !achHistoryView;
          achScroll = 0; achDetailIdx = 0;
        } else if (tx < DISPLAY_W/3) {
          if (achScroll > 0) { achScroll--; achDetailIdx = achScroll; }
        } else if (tx > DISPLAY_W*2/3) {
          if (achScroll + 11 < achViewCount()) { achScroll++; achDetailIdx = achScroll; }
        } else {
    ...
    case PorkchopMode::DIAGNOSTICS: {
      static uint8_t  diagTaps = 0;
      static uint32_t diagTapMs = 0;
      if (isTap) {
        uint32_t now = millis();
        if (diagTaps > 0 && now - diagTapMs > 3000) diagTaps = 0; // too slow, reset
        diagTapMs = now;
        if (++diagTaps >= 7) {
          diagTaps = 0;
    ...
    case PorkchopMode::WEBUI_MODE:
      if (isWebInjected) {
        // A remote D-pad press is meant to navigate whichever mode the
        // web page has switched into (those buttons set currentMode
        // directly, so they never reach this case) — while still sitting
        // on this screen there's nothing local for it to do. Without this,
        // every remote tap toggled or killed the AP mid-session.
        break;
      }
      if (isHold) {
        ...
        if (tx < 60 && ty < TOP_BAR_H + 40) {
          // Corner tap: just leave the screen — the AP/server (if running)
          // keeps going in the background, since WebUI::update() runs every
          // loop() regardless of currentMode. Use HOLD to actually stop it.
          currentMode = PorkchopMode::IDLE;
        } else if (WebUI::isActive()) {
          WebUI::stop();
        } else {
          WebUI::start();
        }
...
static char    _resp[400];  // response — BSS (holds /status's larger payload too)
static uint8_t _frameBuf[160*80*2]; // full screen frame, built up front then streamed

// ── Non-blocking chunked response sender ────────────────────────────────────
// Both /screen and /download can be large enough that sending them in one
// blocking call would stall the main loop (and with it, the pig's
// millis()-based animation timing) for the whole transfer. Instead, once a
// transfer is registered here, WebUI::update() calls _pendService() once
// per loop() iteration; each call does at most one non-blocking send()
// attempt and returns immediately, so everything else in loop() keeps
// running at its normal cadence in between chunks. Only one transfer is
// in flight at a time, matching the existing "one connection per loop()
// call" design used for accept().
enum class PendKind : uint8_t { NONE, BUF, FILE_T };
static PendKind _pendKind     = PendKind::NONE;
static int      _pendCfd      = -1;
static const uint8_t* _pendBuf = nullptr;  // source when PendKind::BUF
static File     _pendFile;                 // source when PendKind::FILE_T
static uint32_t _pendPos      = 0;         // bytes consumed from the source so far
static uint32_t _pendTotal    = 0;         // total bytes to send
static uint8_t  _pendChunk[512];
static uint16_t _pendChunkLen  = 0;        // valid bytes currently in _pendChunk
static uint16_t _pendChunkSent = 0;        // bytes of _pendChunk already sent

static void _pendFinish() {
  if (_pendKind == PendKind::FILE_T && _pendFile) _pendFile.close();
  if (_pendCfd >= 0) { shutdown(_pendCfd, SHUT_WR); close(_pendCfd); }
  _pendKind = PendKind::NONE; _pendCfd = -1; _pendBuf = nullptr;
  _pendPos = 0; _pendTotal = 0; _pendChunkLen = 0; _pendChunkSent = 0;
}

// Hand off a RAM buffer (must outlive the transfer — e.g. a static buffer)
// to the chunked sender. Caller has already sent the HTTP header.
static void _pendStartBuf(int cfd, const uint8_t* buf, uint32_t len) {
  fcntl(cfd, F_SETFL, O_NONBLOCK);
  _pendKind = PendKind::BUF; _pendCfd = cfd; _pendBuf = buf;
  _pendPos = 0; _pendTotal = len; _pendChunkLen = 0; _pendChunkSent = 0;
}

// Hand off an already-open SD File — ownership passes to the state machine,
// which closes it when done. Caller has already sent the HTTP header.
static void _pendStartFile(int cfd, File& f, uint32_t len) {
  fcntl(cfd, F_SETFL, O_NONBLOCK);
  _pendKind = PendKind::FILE_T; _pendCfd = cfd; _pendFile = f;
  _pendPos = 0; _pendTotal = len; _pendChunkLen = 0; _pendChunkSent = 0;
}

// Bounded, non-blocking unit of work — safe to call every loop() iteration.
static void _pendService() {
  if (_pendKind == PendKind::NONE) return;

  // Finish draining whatever's left of the current chunk first.
  if (_pendChunkSent < _pendChunkLen) {
    int rv = send(_pendCfd, _pendChunk+_pendChunkSent, _pendChunkLen-_pendChunkSent, 0);
    if (rv > 0) { _pendChunkSent += rv; }
    else if (rv < 0 && errno != EWOULDBLOCK && errno != EAGAIN) { _pendFinish(); }
    return; // one send attempt per tick — never block waiting for more room
  }

  // Chunk fully sent — done with the whole transfer?
  if (_pendPos >= _pendTotal) { _pendFinish(); return; }

  // Refill the chunk from whichever source this transfer uses.
  uint32_t want = _pendTotal - _pendPos;
  if (want > sizeof(_pendChunk)) want = sizeof(_pendChunk);
  if (_pendKind == PendKind::BUF) {
    memcpy(_pendChunk, _pendBuf + _pendPos, want);
  } else {
    // Touch::update() runs on other loop() iterations in between each of
    // our chunk refills, and reasserts its own pin routing on the shared
    // VSPI bus every time it does a transaction — so by the time we get
    // back here, the bus may no longer be pointed at the SD card's pins.
    // Every other SD access in this codebase gets away without this
    // because it opens/reads/closes atomically in one call; streaming a
    // file across many separate ticks means we have to re-reclaim it
    // before every single read, not just the initial open.
    SD_RECLAIM();
    int n = _pendFile.read(_pendChunk, want);
    if (n <= 0) { _pendFinish(); return; }
    want = (uint32_t)n;
  }
  _pendPos += want;
  _pendChunkLen  = (uint16_t)want;
  _pendChunkSent = 0;
}

// Minimal URL-decode (%XX and +) — capture filenames are built from SSIDs,
// which often contain spaces or punctuation.
static void _urlDecode(const char* src, char* dst, size_t dstLen) {
  size_t o=0;
  while (*src && o+1<dstLen) {
    if (*src=='%' && src[1] && src[2]) {
      char h[3]={src[1],src[2],0};
      dst[o++]=(char)strtol(h,nullptr,16);
      src+=3;
    } else if (*src=='+') { dst[o++]=' '; src++; }
    else { dst[o++]=*src++; }
  }
  dst[o]=0;
}

// Extract a raw (still-encoded) query-string value, e.g. key="dir=" for
// "GET /files?dir=%2Fcaptures HTTP/1.1".
static bool _queryParam(const char* req, const char* key, char* out, size_t outLen) {
  const char* q = strchr(req,'?');
  if (!q) return false;
  const char* kv = strstr(q, key);
  if (!kv) return false;
  kv += strlen(key);
  uint16_t i=0;
  while (*kv && *kv!=' ' && *kv!='&' && i<outLen-1) out[i++]=*kv++;
  out[i]=0;
  return true;
}

// Rudimentary path guard for the file browser — must be an absolute path,
// no ".." components. This is a LAN-only, single-user device; it isn't
// hardened against a deliberately hostile client.
static bool _sanitizePath(const char* p) {
...
static void _serveClient(int cfd) {
  ...
  for (int req=0; req<8; req++) {
    ...
    bool isStatus  = (strncmp(_rbuf,"GET /status",11)==0);
    bool isFiles   = (strncmp(_rbuf,"GET /files",10)==0);
    bool isDownload= (strncmp(_rbuf,"GET /download",13)==0);
    bool isManifest= (strncmp(_rbuf,"GET /manifest.json",18)==0);
    bool isIcon    = (strncmp(_rbuf,"GET /icon.svg",13)==0);
    ...
    if (isOptions) {
      ...
      if (_pendKind != PendKind::NONE) {
        // Another chunked transfer (screen or download) is already in
        // flight — only one at a time. The JS retries on its own schedule.
        const char* busy="HTTP/1.1 503 Service Unavailable\r\nRetry-After: 1\r\nContent-Length: 0\r\n\r\n";
        send(cfd,busy,strlen(busy),0);
      } else {
      const uint16_t sprH=(uint16_t)mainSprite.height();
      const uint16_t outW=160, outH=(sprH>=200)?100:(sprH/2>0?sprH/2:80);
      ...
      send(cfd, hdr, strlen(hdr), 0);
      // mainSprite is 8bpp/palette-indexed (see Display::init()'s RAM-saving
      // comment), so its raw buffer is 1 byte/pixel — NOT 2-byte RGB565.
      // readPixel() resolves the palette correctly regardless of color
      // depth; a prior raw-pointer "fast path" here misread the buffer as
      // 16bpp, which corrupted the mirrored image. Always go through the
      // safe, palette-aware accessor.
      //
      // Downsampling at 4x crushed 6x8px characters into ~1.5x2 output
      // pixels — not enough resolution for a letterform to survive under
      // ANY sampling method. At 2x, characters get ~3x4 output pixels,
      // which is legible. Each 2x2 source block still gets scanned and
      // shown as "lit" if any sample in it isn't background, so thin 1px
      // strokes aren't lost to point-sampling gaps either.
      //
      // The pixel math below is pure RAM reads (fast, no blocking) — build
      // the ENTIRE frame into _frameBuf first, then hand it to the
      // non-blocking chunked sender. Previously this looped row-by-row
      // with a blocking send() per row, which stalled the whole main loop
      // (and the pig's animation timing with it) for the full transfer.
      uint16_t bg = colorBG();
      uint32_t idx = 0;
        ...
        for(uint16_t x=0;x<outW;x++){
          ...
          for (uint8_t dy=0; dy<2; dy++) {
            ...
            for (uint8_t dx=0; dx<2; dx++) {
              uint16_t s = mainSprite.readPixel(x*2+dx, y*2+dy);
              if (s != bg) { px = s; found = true; break; }
            }
            if (found) break;
          }
          _frameBuf[idx++]=px>>8; _frameBuf[idx++]=px&0xFF;
        }
      }
      Serial.printf("[WEBUI] screen: %ux%u queued=%lu\n",outW,outH,bodyLen);
      _pendStartBuf(cfd, _frameBuf, bodyLen);
      return; // ownership of cfd now belongs to the chunked sender
      }

    } else if (isStatus) {
      uint32_t upSec = millis() / 1000;
      char gpsBuf[24];
      if (gpsHasFix) snprintf(gpsBuf,sizeof(gpsBuf),"%d sat",(int)gpsSats);
      else           snprintf(gpsBuf,sizeof(gpsBuf),"no fix");
      int hap = Mood::getEffectiveHappiness();
      const char* moodStr = (hap>70)?"hype":(hap>30)?"good":(hap>-10)?"ok":(hap>-50)?"meh":"sad";
      char body[280];
      snprintf(body,sizeof(body),
        "mode=%s\nlevel=%u\nxp=%lu\nprogress=%u\nmood=%s\nach=%u\n"
        "nets=%lu\nhs=%lu\npmkid=%lu\ndeauth=%lu\n"
        "gps=%s\nheap=%lu\nuptime=%lu:%02lu:%02lu\n",
        porkchopModeName(currentMode),
        XP::getLevel(), (unsigned long)XP::getTotalXP(),
        (unsigned)XP::getProgress(), moodStr,
        (unsigned)(XP::getAchievementCount()+XP::getAchievementCount2()),
        (unsigned long)XP::getLNets(), (unsigned long)XP::getLHS(),
        (unsigned long)XP::getLPMKID(), (unsigned long)XP::getLDeauth(),
        gpsBuf, (unsigned long)ESP.getFreeHeap(),
      ...
      char rawDir[96], dir[96];
      if (!_queryParam(_rbuf,"dir=",rawDir,sizeof(rawDir))) strcpy(rawDir,"%2F");
      _urlDecode(rawDir, dir, sizeof(dir));
      if (!_sanitizePath(dir)) strcpy(dir,"/");

      static char body[1536];
      size_t blen=0; body[0]=0;
      if (!sdAvailable) {
        blen = snprintf(body,sizeof(body),"ERR|no sd card\n");
      } else {
        SD_RECLAIM();
        File root = SD.open(dir);
        if (!root || !root.isDirectory()) {
          blen = snprintf(body,sizeof(body),"ERR|not found\n");
        } else {
          while (true) {
            File entry = root.openNextFile();
            if (!entry) break;
            const char* n = entry.name();
            // SD library sometimes returns a full path, sometimes just the
            // base name depending on FS backend — normalize for display.
            const char* base = strrchr(n, '/'); base = base ? base+1 : n;
            int w = snprintf(body+blen, sizeof(body)-blen, "%s|%s|%lu\n",
              entry.isDirectory()?"D":"F", base, (unsigned long)entry.size());
            entry.close();
            if (w<=0 || blen+(size_t)w>=sizeof(body)-1) break; // stop before overflow
            blen += (size_t)w;
          }
          root.close();
        }
      ...
      char rawPath[96]={0}, path[96];
      _queryParam(_rbuf,"path=",rawPath,sizeof(rawPath));
      _urlDecode(rawPath, path, sizeof(path));
      bool ok = sdAvailable && _sanitizePath(path);
      File f;
      if (ok) { SD_RECLAIM(); f = SD.open(path, FILE_READ); ok = (bool)f && !f.isDirectory(); }
      if (!ok) {
        ...
        snprintf(hdr,sizeof(hdr),
          ...
          "Access-Control-Allow-Origin: *\r\nConnection: keep-alive\r\n"
          "Content-Length: %lu\r\n\r\n", base, (unsigned long)fsize);
        send(cfd, hdr, strlen(hdr), 0);
        _pendStartFile(cfd, f, fsize);
        return; // ownership of cfd now belongs to the chunked sender
      }
      }

    } else if (isManifest) {
      size_t len = strlen_P(MANIFEST_JSON);
      char hdr[150];
      snprintf(hdr,sizeof(hdr),
        "HTTP/1.1 200 OK\r\nContent-Type: application/manifest+json\r\n"
        "Cache-Control: no-store\r\nConnection: keep-alive\r\nContent-Length: %u\r\n\r\n",(unsigned)len);
      send(cfd,hdr,strlen(hdr),0);
      char buf[400]; strncpy_P(buf, MANIFEST_JSON, sizeof(buf)-1); buf[sizeof(buf)-1]=0;
      send(cfd,buf,len,0);

    } else if (isIcon) {
      size_t len = strlen_P(ICON_SVG);
      char hdr[150];
      snprintf(hdr,sizeof(hdr),
        "HTTP/1.1 200 OK\r\nContent-Type: image/svg+xml\r\n"
        "Cache-Control: max-age=86400\r\nConnection: keep-alive\r\nContent-Length: %u\r\n\r\n",(unsigned)len);
      send(cfd,hdr,strlen(hdr),0);
      char buf[700]; strncpy_P(buf, ICON_SVG, sizeof(buf)-1); buf[sizeof(buf)-1]=0;
      send(cfd,buf,len,0);
...
void start() {
  ...
  // NetworkRecon channel-hops via esp_wifi_set_channel() on every update()
  // call whenever it's running, completely independent of currentMode —
  // that includes while sitting on the WEBUI_MODE screen, since it isn't
  // one of the modes excluded from the idle/menu update branch. Changing
  // the radio's channel out from under an active softAP immediately kicks
  // any connected client off, which is why the AP wouldn't stay connected.
  // Pause it for the duration, same as BaconMode/PigSync already do when
  // they need exclusive use of the radio.
  _reconWasRunning = NetworkRecon::isRunning();
  if (_reconWasRunning) NetworkRecon::pause();
  ...
  listen(_listenSock,3);
  webUIActive=true;
  Serial.printf("[WEBUI] listening on port %d  heap=%u\n", WEBUI_PORT, ESP.getFreeHeap());
  Display::showToast("192.168.4.1  PW:" WEBUI_AP_PASS,4500);
}

void stop() {
  if (!webUIActive) return;
  _pendFinish(); // don't leak a socket or leave an SD file open mid-transfer
...
void update() {
  ...
  // A chunked /screen or /download transfer in progress? Do one small,
  // non-blocking unit of work — this is what keeps large transfers from
  // ever stalling the main loop (and the pig's animation timing) again,
  // regardless of image size or file size.
  //
  // Previously this returned right after, which meant NO new connections
  // could be accepted for the entire (now deliberately spread-out)
  // duration of a transfer — a /cmd button press landing in that window
  // just sat unaccepted in the OS backlog until the transfer finished,
  // which is exactly the "buttons lag behind the screen" symptom. Quick
  // requests shouldn't have to wait behind a slow one, so fall through and
  // still try to accept a new connection this same tick. _serveClient()
  // guards against a second /screen or /download landing mid-transfer.
  if (_pendKind != PendKind::NONE) _pendService();
  ...
  int sndbuf=2048; setsockopt(cfd,SOL_SOCKET,SO_SNDBUF,&sndbuf,sizeof(sndbuf));
...
void setup() {
  ...
  pinMode(SD_CS_PIN, OUTPUT); digitalWrite(SD_CS_PIN, HIGH);
  delay(10);
  sdSPI.begin(18, 19, 23, SD_CS_PIN);  // CLK, MISO, MOSI, CS
  ...
  if (!sdAvailable) {
    ...
    // Confirm they're actually there rather than trusting mkdir's return —
    // a write-protected or corrupt card can report a mounted filesystem
    // but still fail to create directories on it.
    sdFoldersOk = SD.exists("/captures") && SD.exists("/wigle") && SD.exists("/logs");
    if (!sdFoldersOk) Serial.println("[SD] one or more required folders missing after mkdir attempt");
  }
